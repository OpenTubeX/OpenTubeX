...
| `yt-dlp` playback and downloads | YouTube or the media site you open, its media hosts, and the configured proxy, if any | IP address, requested page and media resources, video identifier, formats, and timing. Media hosts also receive the request headers and cookies needed for extracted streams. OpenTubeX's proxy setting is passed to `yt-dlp`. |
