...
test('uses the default percentage threshold', () => {
  assert.equal(DEFAULT_WATCHED_PERCENTAGE_THRESHOLD, 90)
  assert.equal(hasReachedWatchedThreshold(9 * 60, 10 * 60), true)
  assert.equal(hasReachedWatchedThreshold(53 * 60, 60 * 60), false)
  assert.equal(hasReachedWatchedThreshold(54 * 60, 60 * 60), true)
})

test('uses a configurable watched percentage threshold', () => {
  assert.equal(hasReachedWatchedThreshold(0, 10 * 60, 0), true)
  assert.equal(hasReachedWatchedThreshold(3 * 60, 10 * 60, 50), false)
  assert.equal(hasReachedWatchedThreshold(5 * 60, 10 * 60, 50), true)
  assert.equal(hasReachedWatchedThreshold(10 * 60, 10 * 60, 100), true)
})

test('handles zero and one hundred percent thresholds', () => {
...
test('rejects invalid video progress and falls back from invalid thresholds', () => {
  ...
  assert.equal(hasReachedWatchedThreshold(53 * 60, 60 * 60, 101), false)
})
