...
export default defineComponent({
  ...
  setup: function (props, { emit, expose }) {
    ...
    function shouldAutoPipNow() {
      ...
      // keep it open while paused as long as the tab trigger still applies.
    ...
    watch(autoPictureInPictureOnTabChange, () => {
      updateAutoPip()
    })

    // #endregion auto picture-in-picture

    // #region scroll mini player
    ...
    function isNativePipActive() {
      return Boolean(ui?.getControls?.()?.isPiPEnabled?.())
    }

    function isNativeFullscreenActive() {
      return Boolean(ui?.getControls?.()?.isFullScreenEnabled?.())
    }
    ...
    watch(() => store.getters.getScrollMiniPlayerSavedRect, () => {
      loadScrollMiniPlayerSavedRectFromSettings()
    }, { immediate: true })

    watch(scrollMiniPlayerEnabled, () => {
      updateScrollMiniPlayer()
    })
