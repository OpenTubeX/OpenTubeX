...
- For every mobile-related change, verify the affected behavior on an Android emulator using a current debug APK before considering the work done. Report the emulator API level, WebView version, and observed result; Electron tests and mobile-sized desktop viewports do not replace this check.
